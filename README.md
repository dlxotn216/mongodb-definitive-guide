...
* db.currentOp()
  * opid: 작업의 고유 식별자
    * 작업 강제종료시 opid를 명시
  * active: 작업 실행 여부
    * false라면 다른 작업 수행을 위해 양보 혹은 락 획득 대기 중
  * secs_running
    * 작업 수행 시간
  * microsecs_running
    * 작업 수행시간 마이크로초
  * op: 작업 유형
    * query, insert, update, update, remove
  * desc: 클라이언트 식별자
  * locks: 획득한 락 타입
  * waitingForLock
  * numYields: 락 해제하고 우위를 양보한 횟수
    * 주로 쿼리, 갱신, 제거는 다른 작업에게 양보 함
* db.killOp(123)
  * 해당 작업이 우위를 양보했을 때 강제종료 가능
  * 갱신, 조회, 제거는 모두 강제종료 가능

**유령 작업 방지**
* 아래와 같은 유형에서 사용자가 자업 정지해도 새로운 갱신이 계속 나타나느 경우가 있음
  * 컬렉션에 데이터를 대량으로 로드하는 경우
  * 수천개의 갱신을 유발하는 작업 수행 시
* 미확인 쓰기를 사용해서 데이터를 로드하면 쓰기가 소켓 버퍼에 쌓여버림
* 확인 쓰기를 하는 방법으로 유령 작업 방지 가능
  * 버퍼에 쌓일 때 까지 대기하는게 아니라 작업 완료 될 때 까지 쓰기가 대기하게 하는 것
